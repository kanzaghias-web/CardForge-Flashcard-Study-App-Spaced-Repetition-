...
export const EASE_EASY_BONUS = 0.05; // gentler ease growth on "easy" answers (see #6)
...
if (quality === 0) {
  ...
  // Ease adjustment: use a gentler bonus on "easy" (quality 3) so the
  // ease factor grows smoothly instead of ballooning intervals (#6).
  const delta = quality === 3 ? EASE_EASY_BONUS : (quality - 2) * EASE_DELTA;
  ease = ease + delta;
